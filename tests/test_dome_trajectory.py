...
class Harness:
    def __init__(self, initial_state, config_dir=None):
        ...
        self.csc = ATDomeTrajectory.ATDomeTrajectory(
...
class ATDomeTrajectoryTestCase(asynctest.TestCase):
    ...
    async def test_simple_follow(self):
        ...
        async with Harness(initial_state=salobj.State.ENABLED) as harness:
            ...
            az_cmd_state = await harness.dome_remote.evt_azimuthCommandedState.next(
                flush=False, timeout=STD_TIMEOUT
            )
            self.assertEqual(az_cmd_state.commandedState, 1)  # 1=Unknown

            alt_deg = 40
            min_daz_to_move = harness.csc.algorithm.max_daz.deg / math.cos(
                alt_deg * RAD_PER_DEG
            )
    ...
    async def test_configuration(self):
        async with Harness(
            ...
            self.assertEqual(state.summaryState, salobj.State.DISABLED)
            settings = await harness.remote.evt_algorithm.next(
                flush=False, timeout=STD_TIMEOUT
            )
    ...
    async def assert_dome_az(self, harness, expected_az, move_expected):
        ...
        if move_expected:
            ...
            with self.assertRaises(asyncio.TimeoutError):
                await harness.dome_remote.evt_azimuthCommandedState.next(
    ...
    async def check_move(self, harness, az_deg, alt_deg):
        ...
        scaled_daz_deg = (az_deg - harness.dome_csc.cmd_az.deg) * math.cos(
            alt_deg * RAD_PER_DEG
        )
        ...
        harness.atmcs_controller.evt_target.set_put(
        ...
        await asyncio.wait_for(
            self.wait_dome_move(harness, az_deg), timeout=LONG_TIMEOUT
        )
    ...
    async def wait_dome_move(self, harness, az_deg):
        ...
        """
        while True:
            curr_pos = await harness.dome_remote.tel_position.next(
